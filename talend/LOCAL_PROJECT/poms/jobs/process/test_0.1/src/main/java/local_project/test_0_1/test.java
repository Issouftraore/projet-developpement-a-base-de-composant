...
package local_project.test_0_1;
...
public class test implements TalendJob {
	...
	public static class extractStruct implements routines.system.IPersistableRow<extractStruct> {
		final static byte[] commonByteArrayLock_LOCAL_PROJECT_test = new byte[0];
		static byte[] commonByteArray_LOCAL_PROJECT_test = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_LOCAL_PROJECT_test.length) {
					if (length < 1024 && commonByteArray_LOCAL_PROJECT_test.length == 0) {
						commonByteArray_LOCAL_PROJECT_test = new byte[1024];
					} else {
						commonByteArray_LOCAL_PROJECT_test = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_LOCAL_PROJECT_test, 0, length);
				strReturn = new String(commonByteArray_LOCAL_PROJECT_test, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_LOCAL_PROJECT_test, 0, length);
				strReturn = new String(commonByteArray_LOCAL_PROJECT_test, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				fileName_tFileOutputDelimited_1 = (new java.io.File(
						"C:/Users/izila/Downloads/TOS_DI-20211109_1610-V8.0.1/workspace/extract.csv")).getAbsolutePath()
								.replace("\\", "/");
...
// ###############################
				...
				final String decryptedPassword_tDBInput_2 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:Xsh5LtYmyxVrSnrYPUNIRvcBMmTbmZxP1Eua/g==");
				...
				String url_tDBInput_2 = "jdbc:mysql://" + "localhost" + ":" + "3306" + "/" + "dabc" + "?"
						+ properties_tDBInput_2;
				...
				String dbquery_tDBInput_2 = "SELECT \n  `product`.`id`, \n  `product`.`description`, \n  `product`.`name`, \n  `product`.`price`,\n  (SELECT SUM(`pr"
						+ "ice`) FROM `product`) AS `total_price`\nFROM `product`";
				...
				try {
					...
					while (rs_tDBInput_2.next()) {
						...
						if (colQtyInRs_tDBInput_2 < 2) {
							...
							row1.description = routines.system.JDBCUtil.getString(rs_tDBInput_2, 2, false);
						}
						if (colQtyInRs_tDBInput_2 < 3) {
							row1.name = null;
						} else {

							row1.name = routines.system.JDBCUtil.getString(rs_tDBInput_2, 3, false);
						}
						if (colQtyInRs_tDBInput_2 < 4) {
							row1.price = null;
						} else {

							row1.price = rs_tDBInput_2.getDouble(4);
							if (rs_tDBInput_2.wasNull()) {
								row1.price = null;
							}
						}
...
// # Output table : 'extract'
							extract_tmp.id = row1.id;
							extract_tmp.description = row1.description;
							extract_tmp.name = row1.name;
							extract_tmp.price = row1.price;
							extract = extract_tmp;
// ###############################
...
// Start of branch "extract"
						if (extract != null) {
							...
							sb_tFileOutputDelimited_1.append(extract.id);
							sb_tFileOutputDelimited_1.append(OUT_DELIM_tFileOutputDelimited_1);
							if (extract.description != null) {
								sb_tFileOutputDelimited_1.append(extract.description);
							}
							sb_tFileOutputDelimited_1.append(OUT_DELIM_tFileOutputDelimited_1);
							if (extract.name != null) {
								sb_tFileOutputDelimited_1.append(extract.name);
							}
							sb_tFileOutputDelimited_1.append(OUT_DELIM_tFileOutputDelimited_1);
							if (extract.price != null) {
								sb_tFileOutputDelimited_1.append(extract.price);
							}
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = test.class.getClassLoader()
					.getResourceAsStream("local_project/test_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
...
/************************************************************************************************
 * 57523 characters generated by Talend Open Studio for Data Integration on the
 * 2 septembre 2024, 21:51:39 GMT
 ************************************************************************************************/